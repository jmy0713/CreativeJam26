...
func _update_attack(player: Player) -> void:
	...
	var in_reach := absf(player.global_position.x - global_position.x) <= attack_range + 12.0
...
func _position_combat_parts() -> void:
	sword_area.position.x = 38.0 * direction
	shield_visual.position.x = 16.0 * direction
